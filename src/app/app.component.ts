...
export class AppComponent {
  ...
  private draw() {
    ...
    for (i = vertices.length; i--;) {
      ...
      x = (x * 0.96875 + 0.5) * this.canvas.width * 5;
      y = (y * 0.96875 + 0.5) * this.canvas.height * 5;
      vertices[i] = [x, y];
    }
    ...
    for (i = triangles.length; i;) {
      this.ctx.beginPath();
      let z = 230
      --i; this.ctx.moveTo(vertices[triangles[i]][0]-z, vertices[triangles[i]][1]-z);
      --i; this.ctx.lineTo(vertices[triangles[i]][0]-z, vertices[triangles[i]][1]-z);
      --i; this.ctx.lineTo(vertices[triangles[i]][0]-z, vertices[triangles[i]][1]-z);
      ...
      this.ctx.fillStyle = this.colorArray[Math.floor(Math.random() * this.colorArray.length)]
      this.ctx.fill();
    }
  }
